...
def skeleton_get(img, args):
    ...
    for i in range(img.size(0)):
        ...
        _, binary = cv2.threshold(x, 200/255, 255/255, cv2.THRESH_BINARY_INV)
...
def trainGAN(data_loader, networks, opts, epoch, args, additional):
    ...
    for i in t_train:
        ...
        imgs_content = imgs_content.cuda(args.gpu)
        Ground_Truth = Ground_Truth.cuda(args.gpu)
        #imgs_style = Ground_Truthone-->one
        imgs_style = imgs_style.cuda(args.gpu)
        style_label = style_label.cuda(args.gpu)
        content_label = content_label.cuda(args.gpu)

        imgs_content_skeleton = skeleton_get(imgs_content,args)
        Ground_Truth_skeleton = skeleton_get(Ground_Truth, args)
        ...
        x_fake_ss, offset_loss = G.decode(content_fusion_skeleton, s_ref, skip1, skip2)
        x_rec, _ = G.decode(Gt_fusion_skeleton, s_src, skip1, skip2)

        g_fake_logit, _ = D(x_fake, style_label)
        g_rec_logit, _ = D(x_rec, content_label)
        ...
        c_x_fake, _, _ = G.cnt_encoder(x_fake_ss)
