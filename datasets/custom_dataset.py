...
def make_dataset_catch(dir, class_to_idx, extensions):
    images = []
    dir = os.path.expanduser(dir)
    for main in sorted(class_to_idx.keys()):
        ...
        for target in sorted(class_to_idx.keys()):
            ...
            for fname in sorted(os.listdir(d)):
                path_1 = os.path.join(file_path_main, fname)
                path_2 = os.path.join(file_path_after, fname)
...
class ImageFolerRemapSingle(DatasetFolderSingle):
    ...
    def __getitem__(self, index):
        path1, target ,path2= self.samples[index]

        sample = self.loader(path1)
