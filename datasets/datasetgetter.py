...
# parse classes to use
    ...
    for k in class_to_use:
        tmp_idx = torch.nonzero(tot_targets == k, as_tuple=False)
        ...
        if k == class_to_use[0]:
            ...
            train_tmp_idx_class_2 = train_tmp_idx
    ...
    train_dataset_class_2 = torch.utils.data.Subset(dataset, train_tmp_idx_class_2)
...
def get_dataset1(args):
    ...
    dataset = ImageFolerRemapSingle(img_dir, transform=transform, remap_table=remap_table)
    valdataset = ImageFolerRemap(img_dir, transform=transform_val, remap_table=remap_table)
...
# parse classes to use
    ...
    return train_dataset, val_dataset,train_dataset_class_2
